...
- the source commit identifies the built image;
- the following promotion commit changes only the desired image tag;
...
- bootstrap Argo CD only after replacing the `bootstrap` image tag.
